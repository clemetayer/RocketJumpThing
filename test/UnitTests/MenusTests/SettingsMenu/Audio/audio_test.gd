...
func test_init_values() -> void:
	...
	AudioServer.set_bus_volume_db(
		AudioServer.get_bus_index(VariableManager.MAIN_BUS), MAIN_AUDIO_VAL
	)
	AudioServer.set_bus_mute(AudioServer.get_bus_index(VariableManager.MAIN_BUS), MAIN_AUDIO_MUTE)
	## BGM
	var BGM_AUDIO_VAL := -0.5
	var BGM_AUDIO_MUTE := false
	AudioServer.set_bus_volume_db(AudioServer.get_bus_index(VariableManager.BGM_BUS), BGM_AUDIO_VAL)
	AudioServer.set_bus_mute(AudioServer.get_bus_index(VariableManager.BGM_BUS), BGM_AUDIO_MUTE)
	## main
	var EFFECTS_AUDIO_VAL := 0.0
	var EFFECTS_AUDIO_MUTE := true
	AudioServer.set_bus_volume_db(
		AudioServer.get_bus_index(VariableManager.EFFECTS_BUS), EFFECTS_AUDIO_VAL
	)
	AudioServer.set_bus_mute(
		AudioServer.get_bus_index(VariableManager.EFFECTS_BUS), EFFECTS_AUDIO_MUTE
	)
	# tests
	audio._init_values()
	## main
...
func test_on_MainSlider_value_changed() -> void:
	...
	assert_float(AudioServer.get_bus_volume_db(AudioServer.get_bus_index(VariableManager.MAIN_BUS))).is_equal_approx(
...
func test_on_BGMSlider_value_changed() -> void:
	...
	assert_float(AudioServer.get_bus_volume_db(AudioServer.get_bus_index(VariableManager.BGM_BUS))).is_equal_approx(
...
func test_on_EffectsSlider_value_changed() -> void:
	...
	assert_float(AudioServer.get_bus_volume_db(AudioServer.get_bus_index(VariableManager.EFFECTS_BUS))).is_equal_approx(
