...
#---- PRE/POST -----
func before():
	presets_path = "res://test/UnitTests/MenusTests/SettingsMenu/Audio/presets_audio_mock.tscn"
	.before()
...
func test_generate_cfg_file() -> void:
	assert_str(presets._get_init_selected_cfg()).is_equal(SettingsUtils.settings_presets.audio)


func test_get_config_path() -> void:
	assert_str(presets._get_config_path()).is_equal(SettingsUtils.AUDIO_PRESETS_PATH)
