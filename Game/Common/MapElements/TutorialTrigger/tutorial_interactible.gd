...
func _ready_activate_tutorial() -> void:
	_toggle_active(SettingsUtils.settings_data.gameplay.tutorial_level == SettingsUtils.TUTORIAL_LEVEL.some)
...
func _on_SignalManager_trigger_tutorial(key, _ptime) -> void:
	if key == _key:
		_toggle_active(
			SettingsUtils.settings_data.gameplay.tutorial_level == SettingsUtils.TUTORIAL_LEVEL.all or 
