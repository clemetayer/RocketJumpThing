## This script is a slightly modified version of the "debug_draw" by Zylann : https://github.com/Zylann/godot_debug_draw
...
func _process_3d_lines_delayed_free(_items: Array):
	var i := 0
	while i < len(_lines):
		var d = _lines[i]
		if d.frame <= Engine.get_idle_frames():
			d.node.queue_free()
			_lines.remove(i)
		else:
			i += 1
