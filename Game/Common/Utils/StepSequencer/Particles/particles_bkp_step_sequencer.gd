...
# Note to (maybe) angry future self : Making a base class might be a bad idea, since it would lose the static/spatial/particles extends
"""
{
...
func _ready():
	...
	_material = StepSequencerCommon.get_material_in_children_and_duplicate(self, _params)
