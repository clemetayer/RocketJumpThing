...
func _check_is_on_floor() -> bool:
	...
	# 	DebugDraw.set_text("contact_normal", state.get_contact_local_normal(i).angle_to(Vector3.UP))
	# 	if state.get_contact_local_normal(i).angle_to(Vector3.UP) <= PI / 8:
	# 		return true
	# return false
